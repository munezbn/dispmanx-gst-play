...
media_info_cb (GstPlayer * player, GstPlayerMediaInfo * info, GstPlay * play)
{
  ...
  if(video) {
    ...
    if(play->vid_win.vid_width != width || play->vid_win.vid_height != height) {   
      I_LOG_WARNING("?????????? Video Resolution Changed ?????????? %d %d ==> %d %d\n", play->vid_win.vid_width , play->vid_win.vid_height, width, height);
      play->vid_win.vid_width = width;
      play->vid_win.vid_height = height;
      /*once we have video dimensions we can call fullscreen*/
      dispmanx_win_set_fullscreen(&play->vid_win, TRUE);
    }
  }
}
...
static GstBusSyncReply bus_sync_handler (GstBus * bus, GstMessage * message, gpointer user_data)
{
    ...
    GstPlayerVideoOverlayVideoRenderer *renderer_overlay = NULL;

    if (!gst_is_video_overlay_prepare_window_handle_message (message))
        return GST_BUS_PASS;

    I_LOG_DEBUG("Rcived bus_sync_handler for prepare window handle : %d\n", play->video_window_handle);
    if (play->video_window_handle != 0)
    {   
			renderer_overlay = GST_PLAYER_VIDEO_OVERLAY_VIDEO_RENDERER(play->renderer);
			if(renderer_overlay)
			{
				gst_player_video_overlay_video_renderer_set_render_rectangle(renderer_overlay, 0, 0, 1920, 1080);
				gst_player_video_overlay_video_renderer_expose(renderer_overlay);
				play->vid_win.in_fullscreen = TRUE;
			}

    }   
    else
    {   
        I_LOG_WARNING("!!!!!!!!!! Should have obtained video_window_handle by now !!!!!!!!!!\n");
    }   
    return GST_BUS_DROP;
}
#endif
...
play_new (gchar ** uris, gdouble initial_volume)
{
  ...
  if (dispmanx_create_video_window(play) == FALSE) {   
  ...
  else
  {
    ...
    play->renderer = gst_player_video_overlay_video_renderer_new (play->video_window_handle);
    renderer_overlay = GST_PLAYER_VIDEO_OVERLAY_VIDEO_RENDERER(play->renderer);
    if(renderer_overlay)
    {
      /*gst_player_video_overlay_video_renderer_set_render_rectangle(renderer_overlay, 0, 0, 1920, 1080);
  ...
  play->player =
      gst_player_new (play->renderer, gst_player_g_main_context_signal_dispatcher_new
      (NULL));
  play->pipeline = gst_player_get_pipeline (play->player);
  /*bus = gst_element_get_bus (play->pipeline);
  gst_bus_set_sync_handler (bus, (GstBusSyncHandler) bus_sync_handler, play, NULL);*/
  ...
  /*g_object_set(G_OBJECT(pipeline), "force-aspect-ratio", FALSE, NULL);*/
...
keyboard_cb (const gchar * key_input, gpointer user_data)
{
  ...
  switch (g_ascii_tolower (key_input[0])) {
    ...
    case 'Q':
      ...
      break;
    case 'w':
      dispmanx_win_move(&play->vid_win, 0, -(WIN_MOVE_STEPS));
      break;
    case 's':
      dispmanx_win_move(&play->vid_win, 0, WIN_MOVE_STEPS);
      break;
    case 'a':
      dispmanx_win_move(&play->vid_win, -(WIN_MOVE_STEPS), 0);
      break;
    case 'd':
      dispmanx_win_move(&play->vid_win, WIN_MOVE_STEPS, 0);
      break;
    case 'r':
			if(play->vid_win.ar == PLAYER_AR_STRETCH)
					dispmanx_win_set_aspect_ratio(play, PLAYER_AR_ORIGINAL);
			else
					dispmanx_win_set_aspect_ratio(play, PLAYER_AR_STRETCH);
      break;
    case 'f':
			if (play->vid_win.in_fullscreen)
			{                     
				I_LOG_TRACE("Un-FullScreen\n");
				dispmanx_win_set_fullscreen(&play->vid_win, FALSE);
			}
			else
			{
      ...
      break;
    case 'b':
			if(play->bg.opacity != 0)
				dispmanx_win_show_background_element(&play->bg, FALSE);
			else
				dispmanx_win_show_background_element(&play->bg, TRUE);
      break;
    case '>':
      if (!play_next (play)) {
